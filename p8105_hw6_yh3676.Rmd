...
* my_model: lm(bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken)
...
* This violin plot provides a visual comparison of the RMSE distribution across the three different models that established previously. To determine the superior model, the criterion we adapted here is selecting the model that demonstrates lower RMSE values consistently across different cross-validation folds. According to the graph,  `my model` has lower RSME on average compared to `model 1` and `model 2`, suggesting that `my model` has the best predictive accuracy among the three models. 
